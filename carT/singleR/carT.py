...
def run_plot(rds, UMI_tsne, sample, outdir, meta):
    cmd = (
        f'Rscript /SGRNJ03/randd/cjj/Script/carT/singleR/carT.R '
        f'--rds {rds} '
        f'--UMI_tsne {UMI_tsne} '
        f'--sample {sample} '
        f'--outdir {outdir} '
...
def main():
    ...
    meta = glob.glob(f"{os.path.dirname(rds)}/*_metadata.tsv")[0]
    run_plot(rds, UMI_tsne, sample, outdir, meta)
