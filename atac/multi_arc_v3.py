...
def prepare_sjm(mapfile, reference):
    ...
    cmd source activate celescope3.0.0; celescope rna sample --outdir ./{sample}/rna_dir/00.sample --sample {sample} --thread 4 --fq1 {rna_fq_dict[sample][0]}
job_end

job_begin
    name rna_barcode_{sample}
    sched_options -w n -cwd -V -l vf=5g,p=1
    cmd source activate celescope3.0.0; celescope tag barcode --outdir ./{sample}/rna_dir/01.barcode --sample {sample} --thread 4 --fq1 {rna_fq_dict[sample][0]} --fq2 {rna_fq_dict[sample][1]}
job_end
...
""")
    ...
    sched_options -w n -cwd -V -l vf=1g,p=1
    cmd source activate celescope_atac1.4.0; celescope atac sample --outdir ./{sample}/atac_dir/00.sample --sample {sample} --thread 8 --chemistry auto --wells 384 --fq2 {atac_fq_dict[sample][1]}
job_end

job_begin
    name atac_barcode_{sample}
    sched_options -w n -cwd -V -l vf=5g,p=1
    cmd source activate celescope_atac1.4.0; celescope atac barcode --outdir ./{sample}/atac_dir/01.barcode --sample {sample} --thread 8 --chemistry auto --lowNum 2 --allowNoLinker --wells 384 --fq1 {atac_fq_dict[sample][0]} --fq2 {atac_fq_dict[sample][1]} --fq3 {atac_fq_dict[sample][2]}
job_end
...
""")
    ...
    sched_options -w n -cwd -V -l vf=5g,p=1
    cmd source activate cjj_atac; python /SGRNJ06/randd/USER/cjj/Script/Self-Used-Scripts/atac/convert_v3.py --rna_path ./{sample}/rna_dir/01.barcode/ --atac_path ./{sample}/atac_dir/01.barcode/ --outdir ./{sample}/convert --sample {sample}
job_end
